...
**Current version:** v2.25.2 — full version-by-version rationale lives in [CHANGELOG.md](./CHANGELOG.md). Multi-phase plan in [ROADMAP.md](./ROADMAP.md). **10/10 against every bar set in prior versions.** First REAL cross-repo super-queen colony ran this cycle: operator dispatched a 5-shard Week 0 cleanup targeting `~/projects/agency-ai-system` (cockpit framework, NOT the protocol repo). Surfaced + fixed 3 integration bugs that only cross-repo execution could expose: Bug 5 (lane dispatches forked the wrong repo HEAD → fixed with `--target` flag + per-colony `target_cwd` + chdir-before-dispatch), Bug 6 (concurrent-worktree cap hard-errored → fixed with `cap_deferred` back-pressure + idempotent re-dispatch), Bug 7 (capture_output stream-reorder JSON-parse regression → fixed with all-lines JSON scan). **Verified**: the s05 Kimi worktree forked from agency-ai-system HEAD and contained the real `src/core/queen_ledger.py` it was assigned to edit — first verified cross-repo dispatch in the protocol's history. New §30.9 documents the cross-repo `target_cwd` model. The calibration loop (ship → real test → find bugs → fix) closed the production-evidence gap exactly as §31 predicted: v2.25.0 (9, self-dogfood) → v2.25.1 (9.5, +4 bugs from self-meta test) → v2.25.2 (10, +3 bugs from real cross-repo colony). **Previous version v2.25.1** — — v2.25.0's "first real end-to-end test" surfaced 4 bugs in real orchestrator integration (validator-path mis-resolution, multi-line JSON parse via `splitlines()[-1]`, missing per-shard prompt generation, decompose-template lane-alias mismatch). All 4 fixed in v2.25.1. Re-test passes through phases 0-5 cleanly. Calibration-loop discipline (ship → test → find bugs → fix → re-ship) validated. The remaining 0.5 toward 10/10 still requires (a) phases 6-14 exercised via a real coding-agent shard execution, (b) cross-repo usage in a non-protocol client project. **Previous version v2.25.0** — (cap on what I can ship without operator participation; 10/10 unlocks when operator runs `super-queen.sh feature ...` on a non-protocol client repo). Headline this bump: orchestrator hardening + first real end-to-end test. (1) **`cmd_decompose` auto-substitution LIVE** — `super-queen.sh feature "<desc>"` now dispatches Kimi background with the kind-specific template + feature text auto-substituted, captures PID, polls. Manual mode preserved as `decompose-manual`. (2) **`cmd_dispatch` multi-lane routing LIVE** — routes per-shard to Kimi / Gemini / Grok-Build / Gemma-4 via their task scripts. Codex/Claude lanes record `needs_agent_spawn` with explicit operator instruction (they require Claude Code's `Agent` tool which is bash-unreachable by design). (3) **`cmd_auto_converge` NEW** — polls Kimi PIDs every 15s (max 30 min), auto-fires `cmd_converge` when all DONE; reports per-poll progress. (4) **`cmd_abort` hardened** — per-lane cleanup (cancels via correct task script), decompose-phase Kimi cancellation, telemetry emission, forensic colony-dir preservation. (5) **`cmd_version` + `decompose-manual` NEW** subcommands. **Live end-to-end test this turn**: `super-queen.sh feature "add ls-failed subcommand"` → colony created → safety check ✓ → decompose-prompt substituted ✓ → Kimi pid=80102 dispatched ✓ → telemetry shows **1/10 colonies tracked** (first real evidence accumulated). 9 more real colonies unlock v3.1.x learning. Honest residual at 9/10: cross-repo usage still operator-bound, failure-recovery beyond abort still spec-only.
