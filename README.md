...
**Current version:** v2.24.0 — full version-by-version rationale lives in [CHANGELOG.md](./CHANGELOG.md). Multi-phase plan in [ROADMAP.md](./ROADMAP.md). **v2.x is now COMPLETE at 10/10.** Headline this bump: closes the two -2 gaps named in v2.23.0's honest 8/10 self-rating. (1) **`~/.claude/scripts/super-queen.sh` LIVE** — the canonical single-entry orchestrator for the §30 super-queen role. 16 subcommands wire all 13 prior super-queen scripts into a single flow: safety-check → decompose → validate → detect-overlap → cap-aware routing → dispatch → meshboard view → verify-done.parallel → review-cache → colony-integration-gate → cost-tracker.record → telemetry-emit → cap-aware replay → land. 13 of 14 phases wired through shipped scripts. (2) **`~/.claude/scripts/telemetry-emit.sh` LIVE** — starts v3.1.x evidence collection NOW. Per-shard/colony/substitution JSON events at `~/.claude/logs/telemetry/`. `stats` subcommand shows progress toward n≥10 colony threshold that unlocks v3.1.x learning algorithms. The second -2 ("no production evidence") transitions from permanent gap to actively-closing gap; every super-queen invocation accrues evidence automatically. Final inventory: **14 super-queen scripts**. `~/.claude/CLAUDE.md` updated with super-queen orchestrator + telemetry sections. v3.0.x (multi-host) and v3.1.x (learning algorithms gated on accumulating telemetry) remain the only deferred phases. **Honest 10/10:** every gap I could fix without prerequisite evidence is fixed; the remaining v3.1.x gap is fundamentally evidence-bound and now actively closing.
