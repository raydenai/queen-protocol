...
**Current version:** v2.12.0 (automated colony-watcher daemon — `colony-watcher.sh` runs every 10 min via launchd/cron, auto-normalizes failing reports + seals stale LAND state + flags timeout shards while operator works in parallel tabs. Elev-W1 colony reached 22/22 CONVERGE_AUDIT_PASS this session via REAP + report synthesis).
