...
**Current version:** v2.18.0 — full version-by-version rationale lives in [CHANGELOG.md](./CHANGELOG.md). Headline this bump bundles two changes that belong together: **(a) speed-first defaults** lower the dispatch matrix race threshold from 5+ → 3+ files (any 3-file change now gets parallel Claude+Kimi+Codex implementation race, pick best diff) AND drop the parallel-review threshold to 1+ non-trivial file (single-file >30 LOC or any 2-file change gets parallel Kimi review), with Gemma 4 local routing made explicit for classification work to preserve paid-lane caps; **(b) Antigravity IDE v1.107.0** characterized as the OPERATOR-DRIVEN PARALLEL-QUEEN lane in new §29.19 — *not* a headless sidecar (no `-p`/`--print`/`--output-format` surface; `chat --mode agent` opens an IDE window). Five anti-fixes documented; `ping_antigravity()` wired into `~/.claude/scripts/sidecar-health.sh` with the status banner now grouped 3-way: `[HEADLESS SIDECARS]` (Kimi/Codex/Gemini/Grok-Build), `[SPECIALTY HEADLESS]` (Grok/Jules), `[OPERATOR-DRIVEN LANES]` (Antigravity). 8-way surface live; all lanes report HEALTHY. Operator vision unifying both: parallel-by-default execution + clean topology for multi-queen work — same goal, two halves.
