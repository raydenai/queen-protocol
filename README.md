...
**Current version:** v2.19.2 — full version-by-version rationale lives in [CHANGELOG.md](./CHANGELOG.md). Multi-phase plan in [ROADMAP.md](./ROADMAP.md). Headline this bump: **Wave A COMPLETE** — both speed-implementation levers shipped + v3.x spec docs added. (1) `~/.claude/scripts/cap-autopilot.sh` NEW + live + smoke-tested (lever 8): reads per-lane usage + caps, recommends substitution at near-cap (Codex→Gemini→Gemma-4-local), telemetry log + 60s cache. (2) `~/.claude/scripts/verify-done.parallel.draft.sh` NEW as draft (lever 5): parallelizes 13 Tier-1 sub-tier gates via bash `&`+`wait`, preserves exit contract + skip-gate magic comments, adds timing instrumentation; operator opts in by wiring into Stop hook after A/B test. (3) `docs/v3.0-multi-host-spec.md` NEW (Wave F design: Postgres advisory locks RECOMMENDED, Redis fallback, claude-mesh authoritative state promotion). (4) `docs/v3.1-learning-spec.md` NEW (Wave G evidence-collection plan: per-shard/colony/substitution telemetry schemas, gates v3.1.x algorithms on n≥30 production colonies). Wave A shipped via 3 file-disjoint parallel tracks (Claude in-turn + 2 Kimi background) — first dogfood proof of the §30 super-queen role spec. Wave B (v2.20.x decomposition automation) prompts drafted at `/tmp/v2.20.*-prompt.md`; dispatch queued for next session after concurrent-isolated cap clears.
