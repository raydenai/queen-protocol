...
**Current version:** v2.21.0 — full version-by-version rationale lives in [CHANGELOG.md](./CHANGELOG.md). Multi-phase plan in [ROADMAP.md](./ROADMAP.md). Headline this bump: **Wave C COMPLETE** — cross-queen coordination shipped (bundles planned v2.21.0/v2.21.1/v2.21.2). (1) `~/.claude/scripts/super-queen-cache.sh` NEW + LIVE (lever 7): read-through cache for super-queen to share file contents across child queens (target 40-60% hit rate), 1h TTL, 100MB LRU cap, PostToolUse-triggered invalidation. (2) `~/.claude/scripts/detect-overlap.sh` NEW + LIVE (§30.3 anti-decomposition rule made enforceable): detects overlapping `files_allowed` across queens, recommends MERGE (lock files, package.json, type-exports) or SERIALIZE (feature exports), with `--fix-merge`/`--fix-serialize` opt-in modes. (3) `lib/overlap-test-fixtures/` NEW (5 fixtures covering merge/serialize/no-overlap cases). (4) `docs/v2.21.2-meshboard-spec.md` NEW: full progress-aggregation spec (super-queen-emit + meshboard-aggregator daemon + meshboard-view TUI); implementation deferred. Wave C shipped via 3 file-disjoint parallel tracks — third §30 super-queen dogfood proof; three for three. The trio of v2.19.2 + v2.20.0 + v2.21.0 collectively gives the super-queen: cap-aware routing + auto-decomposition + graph validation + overlap detection + cross-queen cache. The MACHINERY is built; v2.22.x → v2.23.x integrates the FLOW.
