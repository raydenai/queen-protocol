...
**Current version:** v2.19.0 — full version-by-version rationale lives in [CHANGELOG.md](./CHANGELOG.md). Headline this bump: **§30 NEW — the super-queen role specification**. A new ROLE in the protocol's noun-set, sister to §1 (regular queen). Super-queen = meta-orchestrator running in the operator's single chat interface; decomposes feature requests into shard graphs, spawns N child queens in parallel (one per file-disjoint shard group), aggregates results back to one stream. NEVER writes code itself — pure dispatch + aggregate. 8 subsections cover role definition, input contract, decomposition heuristics, the **10 routing-intelligence levers** (tier matching, cost-asymmetric lane routing, quality-from-parallelism, verification reuse, parallel verification gates, speculative dispatch, cross-queen shared cache, cap-aware autopilot, work batching, colony-level integration verification), cross-queen coordination (lock arbitration, shared-file serialization, progress aggregation), unified output contract, six anti-fixes (incl. "don't let super-queen WRITE code"; "don't decompose into overlapping shard groups"), and an honest scope statement: spec ships v2.19.0; parallel verification gates + cap-aware autopilot deferred to v2.19.x; auto-decomposition + speculative dispatch + cross-queen cache + integration verification gate + unified meshboard deferred to v2.20+. Resolves operator's "max speed AND max quality AND resource-wise" challenge: all three on the same axis when matrix tier is matched correctly — not a tradeoff. Adopt-today-with-discipline: operator can run one Claude Code session as orchestrator and spawn child sessions (terminal panes, Antigravity workspaces per §29.19) for child queens; full automation lands progressively v2.19.x → v2.20+.
