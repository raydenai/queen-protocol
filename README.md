...
**Current version:** v2.20.0 — full version-by-version rationale lives in [CHANGELOG.md](./CHANGELOG.md). Multi-phase plan in [ROADMAP.md](./ROADMAP.md). Headline this bump: **Wave B COMPLETE** — decomposition automation shipped (bundles planned v2.20.0/v2.20.1/v2.20.2 because they belong to the same operator-vision deliverable). (1) `lib/decompose-prompts/` NEW: 5 prompt templates (`decompose-feature.md`, `decompose-bug.md`, `decompose-refactor.md`, `decompose-examples.json` with 5 worked examples, `decompose-design.md` rationale) — super-queen uses these to turn "build feature X" into a §4-shaped shard graph. (2) `scripts/validate-shard-graph.sh` NEW + smoke-tested (17.6K): pre-dispatch validator catches §30.3 anti-decomposition rule violations (overlapping `files_allowed`, dependency cycles, tier validity, cap-awareness sanity) at decomposition time, with `--fix` opt-in auto-merge mode. (3) `docs/v2.20.2-speculative-dispatch-spec.md` NEW: full state-machine extension spec for lever 6 (SPECULATIVE_DISPATCHED → CONFIRMED/DISCARDED), confidence threshold rules, plan-change detection, cost-accounting target (discard rate <15%), 7-entry risk catalog with mitigations; implementation deferred. Wave B shipped via 3 file-disjoint parallel tracks — second §30 super-queen dogfood proof; two for two. Speed × quality × resource: cap-autopilot (v2.19.2) + validator (v2.20.0) together gate every dispatch on caps AND graph correctness BEFORE child queens fire.
