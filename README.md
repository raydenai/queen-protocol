...
**Current version:** v2.22.0 — full version-by-version rationale lives in [CHANGELOG.md](./CHANGELOG.md). Multi-phase plan in [ROADMAP.md](./ROADMAP.md). Headline this bump: **Wave D COMPLETE + concurrent-queens hazard patch (field-evidence driven)**. (1) `colony-integration-gate.sh` LIVE (lever 10): runs ONCE on merged colony result, cost N→1, identifies "integration surface" (cross-shard files + API contracts + migrations) and scopes tests. (2) `review-cache.sh` + `review-cache-core.py` LIVE (lever 4): if reviewer-A passed diff X and shard-B's diff is a strict subset of X (exact hunk match, no fuzzy), skip shard-B's review. (3) `verify-done-promote.sh` LIVE (lever 11): A/B test harness for promoting `verify-done.parallel.draft.sh` to live Stop hook with atomic backup/rollback. (4) **`detect-concurrent-queens.sh` LIVE (field-evidence driven addition)**: operator hit a real failure 2026-05-22 where an intermediate WIP commit from one queen wiped uncommitted work from a peer queen; detector now warns on SessionStart when concurrent queens + uncommitted work coexist. (5) `replay-failed-shard.sh` LIVE (v2.23.2 candidate moved up): operator-invokable shard replay with cap-autopilot-recommended lane swap. Wave D shipped via 3 file-disjoint parallel tracks — fourth §30 super-queen dogfood proof; four for four. v2.19.2 → v2.22.0 collectively ships 10 super-queen-specific scripts; the MACHINERY is now substantial. v2.23.x ships the operator-facing UX layer.
